...
python3 -m sglang.bench_one_batch_server --model model_conf/qwen_sigma --base-url http://10.0.0.102:2000 --batch-size 1 --input-len 1024 --output-len 128
...
python -m sglang.bench_one_batch --model-path model_conf/qwen_sigma --load-format dummy  --batch 1 --input-len 1024 --tp 8 --enable-ep-moe --ep-size 8 --enable-dp-attention --data-parallel-size 8