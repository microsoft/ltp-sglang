...
def analyse_results(log_file, gpu_flops):
    ...
    for l in lines:
        ...
        if "Latency Benchmark Device 0" in l:
            ...
            mfu = calculate_mfu(
                model_config, deploy, phase, bsz, seq_len, latency, gpu_flops
            )
            results.append(
                "{},{},{},{},{},{},{},{},{},{}".format(
                    model,
...
def main():
    parser = argparse.ArgumentParser(
        description="Run benchmark with specified configurations"
    )
    parser.add_argument("--gpu", type=str, choices=["h200"], help="GPU type")
    parser.add_argument(
        "--log",
        type=str,
        default="results/raw_output/benchmark.log",
        help="Log file path",
    )
    parser.add_argument(
        "--csv", type=str, default="results/csv/benchmark.csv", help="CSV file path"
    )
