...
from flop_counter import DecodeFlopCounter, PrefillFlopCounter
from mem_counter import DecodeMemCounter, PrefillMemCounter
...
class CounterRegistry:
    ...
    def get_counter(cls, name: str, **kwargs) -> BaseCounter:
        ...
        if name not in cls._counters:
            raise ValueError(
                f"Counter '{name}' not found. Available: {list(cls._counters.keys())}"
            )
