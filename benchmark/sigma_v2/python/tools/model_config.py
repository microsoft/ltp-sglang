...
PRECISION_TO_BYTES = {"fp16": 2, "bf16": 2, "fp8": 1}
...
class ModelConfig:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "ModelConfig":
        ...
        if missing_required:
            raise ValueError(
                f"Missing required fields in config: {', '.join(missing_required)}"
            )
        if not result["name"] in SUPPORTED_MODELS:
            raise ValueError(
                f"Unsupported model type: {result['name']}. Supported models are: {', '.join(SUPPORTED_MODELS)}"
            )
        return cls(**result)
