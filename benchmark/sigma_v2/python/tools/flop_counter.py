...
class FlopCounter(BaseCounter):
    ...
    def moe(
        ...
        **kwargs
        ...
        return (
            ...
            + self.gemm_flops(
                effective_seq_len, hidden_size, moe_intermediate_size * 2
            )  # gateup proj
            + self.gemm_flops(
                effective_seq_len, moe_intermediate_size, hidden_size
            )  # down proj
    ...
    def _mla_qkv_proj(
        ...
        flops = self.gemm_flops(
            seq_len, hidden_size, q_lora_rank + kv_lora_rank + qk_rope_head_dim
        )
        flops += self.gemm_flops(
            seq_len,
            q_lora_rank,
    ...
    def _attn(
        ...
        if qk_rope_head_dim == 0:
            ...
            attention_dim = self._get_mla_attention_dim(
                num_attention_heads, qk_rope_head_dim, qk_nope_head_dim, kv_lora_rank
            )

        return self.gemm_flops(seq_len, cache_len, attention_dim)

    def _get_mla_attention_dim(
        self,
        num_attention_heads: int,
        qk_rope_head_dim: int,
        qk_nope_head_dim: int,
...
class DecodeFlopCounter(FlopCounter):
    ...
    def _mla_kv_proj(
        ...
        flops = self.gemm_flops(
            seq_len, num_attention_heads, kv_lora_rank * qk_nope_head_dim
        )
